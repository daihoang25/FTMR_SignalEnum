# -*- coding: utf-8 -*-
"""
Created on Mon Jan 11 10:44:23 2021
...
def path_reader(feature_input, N_antenas, SNRdB, delta_SNRdB, N_snapshots, M_fbss, training=True):
    if training == True:
...
def training(feature_input, N_antenas, SNRdB, delta_SNRdB, N_snapshots, M_fbss=None):
    model_path, checkpoint_path_Nsignals, dataset_path = path_reader(feature_input, N_antenas, SNRdB, delta_SNRdB, N_snapshots, M_fbss)
    ...
    print('<=========== Loading testset (SNR = %ddB, deta_SNRdB = %ddB, N_antenas = %d, N_snapshots = %d) ============>'\
    ...
    hist_2 = model_2.fit(X_train, y_train, epochs=50, batch_size=128, validation_split=0.01, 
                        verbose=0, callbacks=[
    ...
    for SNRdB in SNRdB_arr:
        ...
        if training_flag:
            ...
            if 'toep' in feature_input:
                ...
                #for M_fbss in np.arange(5, 23, 2):
                for M_fbss in [19]:
                    ...
                    results_tmp.append(acc)
            results[N_snapshots] = results_tmp
        else:
            #if feature_input == 'toep':
            if 'toep' in feature_input:
                acc = testing(feature_input, N_antenas, SNRdB, delta_SNRdB, N_snapshots, M_fbss=None)
