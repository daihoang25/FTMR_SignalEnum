# -*- coding: utf-8 -*-
"""
Created on Tue Aug 25 20:54:09 2020
...
#SNRdB_arr = [20]
acc_aic_fbss = np.zeros(len(SNRdB_arr))
acc_mdl_fbss = np.zeros(len(SNRdB_arr))
acc_ls_mdl_fbss = np.zeros(len(SNRdB_arr))
acc_im_aic_fbss = np.zeros(len(SNRdB_arr))
acc_topsvd_fbss = np.zeros(len(SNRdB_arr))
acc_sorte_fbss = np.zeros(len(SNRdB_arr))

acc_aic_toep = np.zeros(len(SNRdB_arr))
acc_mdl_toep = np.zeros(len(SNRdB_arr))
acc_im_aic_toep = np.zeros(len(SNRdB_arr))
acc_topsvd_toep = np.zeros(len(SNRdB_arr))
acc_sorte_toep = np.zeros(len(SNRdB_arr))
...
for s in range(len(SNRdB_arr)):
    ...
    print('\n<=========== Importing dataset (SNR = %ddB, N_antenas = %d, N_snapshots = %d) ============>'%(SNRdB, N_antenas, N_snapshots))
    
    print('\n<=========== AIC, MDL with FBSS method ============>')
    model_path, checkpoint_path_Nsignals, testset_path = path_reader('fbss', N_antenas, SNRdB, N_snapshots, M_fbss=11, training=False)
    ...
    model_path, checkpoint_path_Nsignals, testset_path = path_reader('toep', N_antenas, SNRdB, N_snapshots, M_fbss=None, training=False)
    data = np.load(testset_path, allow_pickle=True).item()
    ...
    for M in [7, 9, 11]:
    #for M in [8]:
        acc_tmp.append(testing('fbss', N_antenas, SNRdB, N_snapshots, M_fbss=M))
    acc_ecnet.append(acc_tmp)
    print('\n<=========== Proposed ============>')
    acc_toep[s] = testing('toep', N_antenas, SNRdB, N_snapshots, M_fbss=None)
