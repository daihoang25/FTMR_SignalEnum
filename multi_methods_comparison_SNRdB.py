...
for s in range(len(SNRdB_arr)):
    ...
    model_path, checkpoint_path_Nsignals, testset_path = path_reader('toep', N_antenas, SNRdB, delta_SNRdB, N_snapshots, training=False)
    data = np.load(testset_path, allow_pickle=True).item()
    ...
    print('\n<=========== ECNet ============>')
    acc_tmp = []
    acc_tmp.append(testing('fbss', N_antenas, SNRdB, delta_SNRdB, N_snapshots))
